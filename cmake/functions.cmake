...
function(add_compile_options_to_target)
    set(oneValueArgs COMPILER_ID TARGET WARNING_MESSAGE SCOPE)
    set(multiValueArgs WHEN OPTIONS)
    cmake_parse_arguments(ACO "" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})

    if (NOT "${ACO_WHEN}" STREQUAL "")
        if (NOT ${ACO_WHEN})
            return()
        endif()
    endif()
    ...
    if (NOT ACO_SCOPE)
        set(ACO_SCOPE PRIVATE)
    endif()

    set(APPLY_OPTIONS FALSE)
    if (NOT ACO_COMPILER_ID)
        set(APPLY_OPTIONS TRUE)
    elseif ("${CMAKE_CXX_COMPILER_ID}" STREQUAL "${ACO_COMPILER_ID}")
        set(APPLY_OPTIONS TRUE)
    endif()
    ...
    if (APPLY_OPTIONS)
        ...
        target_compile_options(${ACO_TARGET} ${ACO_SCOPE} ${ACO_OPTIONS})
    endif()
endfunction()

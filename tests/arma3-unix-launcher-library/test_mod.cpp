...
TEST_CASE_FIXTURE(ModTests, "MissingQuotes")
{
    ...
    {
        REQUIRE_CALL(filesystemUtilsMock, Ls(remove_stamina_path, _)).TIMES(2).RETURN(std::vector<std::string>{"addons", "mod.cpp"});
        REQUIRE_CALL(stdUtilsMock, FileReadAllText(remove_stamina_mod_cpp_path)).LR_RETURN(remove_stamina_mod_cpp_missing_quotes);
...
TEST_CASE_FIXTURE(ModTests, "NoWhitespaces")
{
    ...
    {
        REQUIRE_CALL(filesystemUtilsMock, Ls(remove_stamina_path, _)).TIMES(2).RETURN(std::vector<std::string>{"addons", "mod.cpp"});
        REQUIRE_CALL(stdUtilsMock, FileReadAllText(remove_stamina_mod_cpp_path)).LR_RETURN(remove_stamina_mod_cpp_no_whitespaces);
